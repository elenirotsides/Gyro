...
router.post('/create', async (req, res) => {
    ...
    try {
        await tasks.addTask(input['taskName'], req.session.user, input['status'], input['tags']);
    } catch (e) {
        res.render('/create', { error: 'Uh oh, something went wrong, pleease try again' });
    }
});
...
router.post('/:id/comments/create', async (req, res) => {
    ...
    if (!input['comment']) {
        res.status(400).json({ error: 'You must write a comment to submit a comment' });
    }

    try {
        await tasks.addComment(id, req.session.user, input['comment']);
    } catch (e) {
        res.render('/:id/comments/create', { error: 'Uh oh, something went wrong! Please try again' });
